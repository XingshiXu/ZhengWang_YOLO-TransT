...
#new是通过修改yolov8.yaml这种配置文件读取模型
    def _new(self, cfg: str, task=None, verbose=True):
        ...
        #################################这个权重一定要跟my_train.py中的yolov8n.yaml的版本对应
        ckpt = torch.load('/media/test/WZ/03_github_yolo_transt/yolov8_transt/yolov8/ultralytics/yolo/v8/detect/yolov8n.pt')
        csd = ckpt['model'].float().state_dict()
        csd = intersect_dicts(csd, self.model.state_dict())
        self.model.load_state_dict(csd, strict=False)
        print(f'Transferred{len(csd)}/{len(self.model.state_dict())} items')

    #load通过yolov8.pth这种预训练权重导入模型
