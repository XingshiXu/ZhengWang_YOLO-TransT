...
class TransT(nn.Module):
    ...
    def forward(self, search, template):  # the size of search:Batchsize, 3, 256, 256     the size of template:Batch, 3, 128, 128
        ...
        """
        if not isinstance(search, NestedTensor):  # NestedTensor:feature + mask
        ...
        feature_search, pos_search = self.backbone(search)  # join(backbone + pos_embed)
...
class SetCriterion(nn.Module):  # loss
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):  # classification
