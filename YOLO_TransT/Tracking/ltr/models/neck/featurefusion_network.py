...
class FeatureFusionNetwork(nn.Module):  # Transformer
...
class FeatureFusionLayer(nn.Module):  # DETR, STARK(这部分是transformer的最基本的结构，不需要看)
    ...
    def forward_post(self, src1, src2,  # template, search  in DETR/STARK only one src
                     src1_mask: Optional[Tensor] = None,
                     src2_mask: Optional[Tensor] = None,
                     src1_key_padding_mask: Optional[Tensor] = None,
                     src2_key_padding_mask: Optional[Tensor] = None,
